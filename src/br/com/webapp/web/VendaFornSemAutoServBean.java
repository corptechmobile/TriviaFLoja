...
@ManagedBean(name = "vendaFornSemAutoServBean")
@SessionScoped
public class VendaFornSemAutoServBean implements Serializable, IMenuAcesso{
	...
	private static final long serialVersionUID = -8015954149977728189L;
	...
	@PostConstruct
	public void init(){
		System.out.println("[VendaFornSemAutoServBean][init]");
		onStartDatas();
		setAutoServicoPedAmbos("pedido");
	...
	public void buscar() {
		...
		lista = vendaFornDTORN.listarPedidoSemAutoServico(empresaFilter, vendedorFilter, null, porFilter, dataFilter1, dataFilter2);
		...
		resumo = vendaFornResumoRN.carregarMesSemAutoServico(empresaFilter, vendedorFilter, null, null, dataFilter2);
		if(resumo!=null) {
		...
		VendaFornResumo vendaFornResumo = vendaFornResumoRN.carregarSemAutoServico(empresaFilter, vendedorFilter, null, null, dataFilter2, dataFilter2);
		...
		if(UtilData.daysBetweenDates(dataFilter1, dataFilter2)>0) {
			vendaFornResumo = vendaFornResumoRN.carregarSemAutoServico(empresaFilter, vendedorFilter, null, null, dataFilter1, dataFilter2);
			//numClientes = vendaFornResumo.getNumClientes();
		}
		ticketMedioDia = 0.0;
		...
		if(diasUteis!=null) {
			if(diasUteis.getPrazoDecorrido() > 0){
				mediaDia = resumo.getFaturamento()/diasUteis.getPrazoDecorrido();
			}
		}
	...
	public List<VendaFornPedVenda> getListaPedidos() {
		if(listaPedidos==null && ((selecionada !=null && selecionadaProduto != null) || selecionadaFPagto != null)) {
			...
			listaPedidos = vendaFornPedVendaRN.listarPedVendaSemAutoServ(selecionada, selecionadaProduto, selecionadaFPagto, empresaFilter, vendedorFilter, dataFilter1, dataFilter2, porFilter);
	...
	public void clearSession() {
		...
		if(request.getParameter("id")!=null){
		    ...
		    session.removeAttribute("vendaFornSemAutoServBean");
		}
	}
